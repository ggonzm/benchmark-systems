...
def double_cart_pendulum(t, x, x_dot, *,
    ...
    F[3] =  p1*ddx + p2*alpha1*np.cos(theta1) + p3*alpha2*np.cos(theta2) - (
        p2*omega1**2*np.sin(theta1) + p3*omega2**2*np.sin(theta2) + u)
    F[4] = -p2*np.cos(theta1)*ddx - p4*alpha1 - p5*alpha2*np.cos(theta1 - theta2) - (
        p7*np.sin(theta1) + p5*omega2**2*np.sin(theta1 - theta2))
    F[5] = -p3*np.cos(theta2)*ddx - p5*alpha1*np.cos(theta1 - theta2) - p6*alpha2 - (
        -p5*omega1**2*np.sin(theta1 - theta2) + p8*np.sin(theta2))
